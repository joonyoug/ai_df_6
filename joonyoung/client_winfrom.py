...
import serial  # 아두이노와의 시리얼 통신을 위한 모듈
...
# 아두이노 포트 설정 (여기 수정)
arduino = serial.Serial('/dev/cu.usbserial-1130', 9600, timeout=1)  # 아두이노 포트 경로 수정

# 메시지 전송 여부를 확인하는 변수
message_sent = False
signal_sent = False  # 아두이노 신호 전송 여부
    ...
    if not frame_queue.empty():
        try:
            ...
            for *box, conf, cls in detections:
                if conf >= 0.8:
                    if not message_sent:  # 메시지가 아직 전송되지 않았다면
                        # SMS 전송
                        sms_sender.send_sms("+821083988226", "test")
                        message_sent = True  # 메시지를 전송한 상태로 변경
                        arduino.write(b'1')  # 아두이노에 신호 보내기
...
btn_exit = Button(frame_buttons, text="테스트", command=test_led, font=("Arial", 12, 'bold'), width=15, bg='#D19A66', fg='#000000')
